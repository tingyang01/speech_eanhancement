...
//===========================================================
template <class T>
std::vector<T> WindowFunctions<T>::createWindow (int numSamples, WindowType windowType, bool fftBins)
{
    if (windowType == HanningWindow)
    {
        std::vector<T> window = createHanningWindow (numSamples, fftBins);
        return window;
    }
...
//===========================================================
template <class T>
std::vector<T> WindowFunctions<T>::createHanningWindow (int numSamples, bool fftBins)
{
    std::vector<T> window (numSamples);
    
	if (fftBins) // periodic windows
	{
		T numSamplesMinus1 = (T)(numSamples);		// the number of samples minus 1
