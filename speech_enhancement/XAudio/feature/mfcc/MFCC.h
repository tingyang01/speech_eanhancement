...
public:
	...
	/** Set the number of mels to calculate
	 * @param nMels_ the number of mels to calculate
	 */
	void setNumMels(int nMels_);
     ...
     */
    void calculateMelFrequencyCepstralCoefficients (const std::vector<T>& magnitudeSpectrum, T energy);
	...
	/** a vector to hold the log spectrum once they have been computed */
	std::vector<T> logSpectrum;
...
private:
     ...
     */
    void discreteCosineTransform (std::vector<T>& inputSignal, const size_t numElements, bool bOrtho = true);
	 ...
	 *
	*/
	void calculateMelFilterBank2();

    /** Calculates mel from frequency
     * @param frequency the frequency in Hz
     * @returns the equivalent mel value
     */
    T frequencyToMel (T frequency, bool htk = false);

	/** Calculates frequency from mel
	 * @param mel
	 * @returns the equivalent frequency value in hz
	 * @htk is bool value,  true when use HTK formula, false when use Slaney
	 * this is made from librosa
	 */
	T melToFrequency(T mel, bool htk = false);

	void PowerToDB(double ref = 1.0, double amin = 1e-10, double top_db = 80.0);
