...
//=======================================================================
template <class T>
Gist<T>::Gist (int audioSampleSize, int numFFT, int numHop, int fs, WindowType windowType_)
 :  fftConfigured (false),
    onsetDetectionFunction (audioSampleSize),
    yin (fs),
    mfcc (audioSampleSize, fs),
    windowType (windowType_)
{
    samplingFrequency = fs;
    setAudioFrameSize (audioSampleSize, numFFT, numHop);
}
...
//=======================================================================
template <class T>
void Gist<T>::setAudioFrameSize (int audioSampleSize, int numFFT, int numHop)
{
    frameSize = audioSampleSize;
    audioFrame.resize (frameSize);
	bool fftBins = true;
    windowFunction = WindowFunctions<T>::createWindow (numFFT, windowType, fftBins);
...
void Gist<T>::processAudioFrame (const T* frame, int numSamples)
{
	...
	energy = 0;
	for (auto v : audioFrame)
		energy += v * v;
	energy = sqrt(energy / audioFrame.size());
...
//=======================================================================
template <class T>
void Gist<T>::processSTFT(const T* frame, int numSamples, int numFFT, int numHop)
{
	audioFrame.reserve(numSamples);
...
{
    mfcc.calculateMelFrequencyCepstralCoefficients (magnitudeSpectrum, energy);
    return mfcc.MFCCs;
}
