...
#else
#define array_exp remez5_0_log2_neon
#endif
...
void GMMTrainerBaseline::iteration(std::vector<std::vector<real_t>> &X) {
	...
	bool enable_guarded_timer = verbosity >= 2;
	{
		...
		for (int i = 0; i < nr_batch; i++)
		{
			...
			memset(buffers[i], 0, sizeof(double)*gmm->gaussians[0]->fast_gaussian_dim);
		}
...
#define PAUSE() \
	...
	real_t last_ll = -1.79769e+308; // numeric_limits<real_t>::max();
